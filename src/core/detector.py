...
from .types import (
    ThreatDetection, ScanResult, SecurityPolicy,
    DetectionMethod, SeverityLevel, AttackCategory
)
    ...
    def scan_text(
        ...
        for pattern in applicable_patterns: 
            ...
            for position, matched_text in matches: 
                ...
                if (policy.block_on_detection and 
    ...
    def _get_applicable_patterns(
        ...
        if policy.categories:
            for category in policy.categories:
                patterns.extend(
                    self.pattern_library.get_patterns_by_category(category)
                )
        else:
            # Use all patterns
